...
var daemon = function(config) {
  ...
  let plistPath = '/Library/LaunchDaemons';

  if (config.runAsAgent) {
    plistPath = '/Library/LaunchAgents';
  }

  if (config.runAsUserAgent) {
    plistPath = process.env.HOME + '/Library/LaunchAgents';
  }
  ...
  Object.defineProperties(this, {
    ...
    cwd: {
      ...
      writable: true,
      configurable: false,
      value: config.cwd || p.dirname(
        ((this.script === undefined) || (this.script === null))
        ? ''
    ...
    logpath: {
      ...
      writable: true,
      configurable: false,
      value: config.logpath || '/Library/Logs/' + (
      this.name || config.name || 'node-scripts')
    },
    ...
    abortOnError: {
      ...
      writable: false,
      configurable: false,
      value: config.abortOnError instanceof Boolean
        ? config.abortOnError
        : false
    },
        ...
        fs.exists(this.plist, function(exists) {
          if (!exists) {
            ...
            var args = [
              ...
              me.wait.toString(),
              '-r',
              me.maxRestarts.toString(),
              '-a',
              (
                me.abortOnError == true
                ? 'y'
                : 'n')
            ];
